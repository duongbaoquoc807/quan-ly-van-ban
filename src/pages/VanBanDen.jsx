// src/pages/VanBanDen.jsx
import React, { useEffect, useState } from 'react';
import { supabase } from '../supabaseClient';
import UploadCongVan from '../components/UploadCongVan';

const TRANG_THAI_OPTIONS = ['Chưa xử lý', 'Đang xử lý', 'Đã xử lý'];
...
export default function VanBanDen() {
  ...
  async function themVanBan(e) {
    e.preventDefault();
    if (!form.so_ky_hieu || !form.ngay_van_ban || !form.co_quan_ban_hanh || !form.trich_yeu) {
      setLoi('Vui lòng điền đủ các trường bắt buộc (Số ký hiệu, Ngày, Cơ quan, Trích yếu).');
      return;
    }

    setDangLuu(true);
    setLoi('');
    const { error } = await supabase.from('cong_van_den').insert([form]);
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-2xl shadow-sm p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">Thêm văn bản đến</h2>
        ...
        <form onSubmit={themVanBan} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="text-sm text-gray-600">Cơ quan ban hành *</label>
            <input
          ...
          <div className="sm:col-span-2">
            ...
            >
              {dangLuu ? 'Đang lưu...' : 'Lưu văn bản đến'}
            </button>
          </div>
        </form>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">
          Danh sách văn bản đến ({danhSach.length})
        </h2>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-gray-500 border-b">
                  ...
                  <th className="py-2 pr-3">Cơ quan</th>
              ...
              <tbody>
                {danhSach.map((vb) => (
                  <tr key={vb.id} className="border-b last:border-0">
                    ...
                    <td className="py-2 pr-3">{vb.co_quan_ban_hanh}</td>
